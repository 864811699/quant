...
class Server:
    ...
    def create_order(self, longShort, CTPAUAskPrice, CTPAUBidPrice, MT5AUAskPrice, MT5AUBidPrice, USDAskPrice, USDBidPrice, spread, realOpenSpread):
        order = struct.POrder()
        ...
        if success:
            ...
            log.warninging("save order fail,pEntrustNo:{}  error:{}".format(order.pEntrustNo, errmsg))
            return False, None
    ...
    def openOrder(self, vol, action, spread, order):
        current_positon = self.getPosition(action, vol)
        ...
        if current_positon < max_position:
            ...
            success,rtnExecOrder = util.send_order_to_server(self.zmqCtpClient,self.strategyConfig["op1"]["symbol"], action, comm.OFFSET_OPEN,
            ...
            if rtnExecOrder.reqSuccess:
                ...
                while True:
                    success,rtnMt5Exec1 = util.send_order_to_server(self.zmqXAUClient, self.strategyConfig["op2"]["symbol"], order.entrustNo, mt5Action, comm.OFFSET_OPEN, xauusd_askQty)
                    self.check_send_status(success, "xau open order")
                    if rtnMt5Exec1.reqSuccess:
                        xau_p = rtnMt5Exec1.order.bidPrice
                        if rtnMt5Exec1.order.status == mt5comm.ORDER_STATUS_AllTrade or rtnMt5Exec1.order.status == mt5comm.ORDER_STATUS_PARTTRADE:
                ...
                usdcnh_bidQty = 0
                while True:
                    success,rtnMt5Exec2 = util.send_order_to_server(self.zmqUSDClient, self.strategyConfig["op3"]["symbol"],
                    ...
                    if rtnMt5Exec2.reqSuccess:
                        ...
                        if rtnMt5Exec2.order.status == mt5comm.ORDER_STATUS_AllTrade or rtnMt5Exec2.order.status == mt5comm.ORDER_STATUS_PARTTRADE:
                ...
                # 汇总通知
                self.notifyApi.send_trade_result(self.strategyConfig["base"]["startSpread"], self.strategyConfig["base"]["rangeSpread"], spread, action, comm.OFFSET_OPEN,
                                                 current_positon + 1)
                openOrderStatus = True
            else:
                log.info("server open order fail,msg:{}".format(rtnExecOrder.errorMsg))
                self.notifyApi.notify_trade_fail(spread, self.strategyConfig["op1"]["symbol"], action, comm.OFFSET_OPEN, self.strategyConfig["op1"]["rate"], rtnExecOrder.errorMsg)
        else:
    ...
    def openAddOrder(self, order):
        ...
        if order.status == 1:
            ...
            xauusd_bidQty = 0
            while True:
                success,rtnMt5Exec1 = util.send_order_to_server(self.zmqXAUClient,self.strategyConfig["op2"]["symbol"], order.entrustNo,
                                                      mt5Action,
                ...
                if rtnMt5Exec1.reqSuccess:
                    ...
                    if rtnMt5Exec1.order.status == mt5comm.ORDER_STATUS_AllTrade or rtnMt5Exec1.order.status == mt5comm.ORDER_STATUS_PARTTRADE:
                        xauusd_askQty -= rtnMt5Exec1.order.bidVol
                        xauusd_bidQty += rtnMt5Exec1.order.bidVol
                        log.info("mt5 open add success,{}  {}  vol:{} ,price:{}".format(self.strategyConfig["op2"]["symbol"], mt5Action, xauusd_bidQty, xau_p))
                    if random(xauusd_askQty, 4) == 0:
                        order.status = 2
                        self.updateOrder(order)
                        self.notifyApi.notify_trade_success(order.spread, self.strategyConfig["op2"]["symbol"], mt5Action,
                                                            comm.OFFSET_OPEN,
                    ...
                    #  发送成交失败通知
                    log.warning("mt5 open add fail,spread:{}, {}  {}  {}  {}".format(order.spread, self.strategyConfig["op2"]["symbol"], mt5Action, xauusd_askQty, rtnMt5Exec1.msg))
        ...
        if order.status == 2:
            ...
            usdcnh_bidQty = 0
            while True:
                success,rtnMt5Exec2 = util.send_order_to_server(self.zmqUSDClient,self.strategyConfig["op3"]["symbol"], order.entrustNo,
                                                      mt5Action,
                                                      comm.OFFSET_OPEN,
                ...
                if rtnMt5Exec2.reqSuccess:
                    ...
                    #  发送成交失败通知
                    log.warning("mt5 open add fail,spread:{}, {}  {}  {}  {}".format(order.spread, self.strategyConfig["op3"]["symbol"], mt5Action, usdcnh_askQty, rtnMt5Exec2.msg))
                    self.notifyApi.notify_trade_fail(order.spread, self.strategyConfig["op2"]["symbol"], mt5Action,
                                                     comm.OFFSET_OPEN,
            ...
            self.updateOrder(order)
            self.notifyApi.send_trade_result(self.strategyConfig["base"]["startSpread"], self.strategyConfig["base"]["rangeSpread"], order.spread, order.longShort, comm.OFFSET_OPEN, self.getPosition(mt5Action))
    ...
    def closeOrder(self, order):
        ...
        if order.status == 3:
            ...
            success,rtnExecOrder = util.send_order_to_server(self.zmqCtpClient,self.strategyConfig["op1"]["symbol"], order.longShort,
                                                   comm.OFFSET_CLOSE,
                                                   self.strategyConfig["op1"]["rate"], order.entrustNo)
            ...
            if rtnExecOrder.reqSuccess:
                ...
                self.notifyApi.notify_trade_fail(order.speard, self.strategyConfig["op1"]["symbol"], order.longShort, comm.OFFSET_CLOSE, self.strategyConfig["op1"]["rate"], rtnExecOrder.errorMsg)
        elif order.status > 3:
            # 获取ctp成交价
        ...
        if order.status == 4:
            ...
            for order in mt5_orders:
                if order.status == 4 and order.openClose == comm.OFFSET_CLOSE and order.symbol == self.strategyConfig["op2"]["symbol"]:
        ...
        if order.status == 5:
            ...
            usdcnh_bidQty = 0
            while True:
                success,rtnMt5Exec2 = util.send_order_to_server(self.zmqUSDClient,self.strategyConfig["op3"]["symbol"], order.entrustNo, mt5Action, comm.OFFSET_CLOSE, self.strategyConfig["op3"]["rate"])
                self.check_send_status(success, "xau close order")
                if rtnMt5Exec2.reqSuccess:
                    usd_p = rtnMt5Exec2.order.bidPrice
                    if rtnMt5Exec2.order.status == mt5comm.ORDER_STATUS_AllTrade or rtnMt5Exec2.order.status == mt5comm.ORDER_STATUS_PARTTRADE:
                        ...
                        usdcnh_bidQty += rtnMt5Exec2.order.bidVol
                        log.info(
                            "mt5 open success,{}  {}  vol:{} ,price:{}".format(self.strategyConfig["op3"]["symbol"], mt5Action, usdcnh_bidQty, usd_p))
                    if random(usdcnh_askQty, 4) == 0:
                        order.status = 6
                        self.updateOrder(order)
                    ...
                    self.notifyApi.notify_trade_fail(self.strategyConfig["op2"]["symbol"], mt5Action, comm.OFFSET_CLOSE, self.strategyConfig["op2"]["rate"], rtnMt5Exec2.msg)
        ...
        while True:
            ...
            if needCloseN == 0:
                ...
                with concurrent.futures.ThreadPoolExecutor() as executor:
                    future1 = executor.submit(self.ctptdApi.CloseAllOrder, self.strategyConfig["op1"]["symbol"], entrustNo)
                    future2 = executor.submit(util.mt5_api_close_all_position, self.strategyConfig["op2"]["symbol"], entrustNo)
                    future3 = executor.submit(util.mt5_api_close_all_position, self.strategyConfig["op3"]["symbol"], entrustNo)
                    rtnMt5Exec1 = future1.result()
                    rtnMt5Exec2 = future2.result()
                    rtnMt5Exec3 = future3.result()
                # 清仓 伦敦金
                # 清仓 汇率
            # log.info("server closed orders {} times,need to close order {}".format(closedN, needCloseN - closedN))
    ...
    def checkShouldCloseOrder(self, ctpMarket, XAUUSDm, USDCNHm):
        ...
        for order in orders:
            ...
            log.info(
                "should_close_order :{} ,pOrder:{} ,current_spread:{},close_spread:{}".format(is_close, order.PID, spread, order.closeSpread))
            if is_close == True:
                self.closeOrder(order)

    def checkShouldOpenOrder(self, ctpMarket, XAUUSDm, USDCNHm):
        is_open, vol, action, spread = util.should_open_order(ctpMarket, XAUUSDm, USDCNHm, self.strategyConfig["startSpread"], self.strategyConfig["rangeSpread"])
        log.info("should_open_order :{} ,current_spread:{},start_spread:{},range_spread:{}".format(is_open, spread, self.strategyConfig["startSpread"], self.strategyConfig["rangeSpread"]))
        if is_open:

            success,order=self.create_order(action,ctpMarket.askPrice1,ctpMarket.bidPrice1,XAUUSDm.askPrice1,XAUUSDm.bidPrice1,USDCNHm.askPrice1,USDCNHm.bidPrice1,spread)
            if not success:
    ...
    def loadStrategy(self):
        ...
        self.StrategyStatus = False
        if self.strategyConfig != 0:
            log.warning("change strategy config,old strategy:{} ,new strategy:{}".format(self.strategyConfig,
                                                                                      config.strategyConfig))
        else:
            log.warning("load strategy config:{} ,new strategy:{}".format(self.strategyConfig))
