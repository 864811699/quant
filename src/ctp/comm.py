...
OFFSET_DICT = {OFFSET_OPEN: "OPEN", OFFSET_CLOSE: "CLOSE",OFFSET_CLOSE_TODAY:"CLOSE",OFFSET_CLOSE_PREV:"CLOSE"}
...
def getLongShortOpenClose(side, openclose):
    # longshort  openclose
    longshort_data = 0
    openclose_data = OFFSET_DICT[openclose]
...
class CTPMD:
    """
    {pDepthMarketData.UpdateTime} - {pDepthMarketData.InstrumentID} - bidPrice:{pDepthMarketData.BidPrice1} - bidVol:{pDepthMarketData.BidVolume1} - askVol:{pDepthMarketData.AskVolume1} - askPrice:{pDepthMarketData.AskPrice1}
    """
    msgID: int =0
    instrumentID: str = ""
    bidPrice1: float = 0.0
...
class RtnExecOrder:
    ...
    errorMsg: str = ""
    order: struct.Order = field(default_factory=struct.Order)
