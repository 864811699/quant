...
class TdImpl(tdapi.CThostFtdcTraderSpi):
    def __init__(self, host, broker, user, password, appid, authcode):
        ...
        self.entrustNo = 0
        self.OrderDict = {}  # entrustno-->struct.order
        self.lock=threading.Lock()
        self.PositionDict = {}  # longshort-> dict{今/昨->持仓数}
    ...
    def updatePosition(self, position):
        with self.lock:
            if position.longShortType in self.PositionDict:
                # print("position exit::{}".format(position))
                self.PositionDict[position.longShortType][position.PositionDate]=position
            else:
                # print(position)
                self.PositionDict[position.longShortType]={}
                self.PositionDict[position.longShortType][position.PositionDate]=position

    def readPosition(self):
        with self.lock:
            position_vol_dict = {comm.ACTION_LONG:0,comm.ACTION_SHORT:0}
            for k,v in self.PositionDict.items():
                vol=0
                for v2 in v.values():
                    vol+=v2.position
    ...
    def OnRspQryInvestorPosition(self, pInvestorPosition: tdapi.CThostFtdcInvestorPositionField,
        ...
        if pInvestorPosition is not None:
            ...
            position.todayPosition = pInvestorPosition.TodayPosition  # 今日持仓
            position.ysdPosition = pInvestorPosition.YdPosition  # 上日持仓
    ...
    def GetPrice(self, ExchangeID, InstrumentID, longshort,openclose):
        ...
        price = 0
        if success == True:
            price = self.priceDict[comm.MARKET_SELL1] if side == comm.SIDE_DICT["BUY"] else self.priceDict[
                comm.MARKET_BUY1]
        return price
    ...
    def OnRtnTrade(self, pTrade):
        ...
        trade.tradeTime = datetime.strptime(pTrade.TradeDate + pTrade.TradeTime,
                                            "%Y%m%d%H:%M:%S")  # date char[9]   time char[9]
        trade.price = pTrade.Price
        trade.volume = pTrade.Volume
        trade.orderRef = pTrade.OrderRef

        self.queueRtnOrder.put(pTrade)
    ...
    def OrderInsert(self, ExchangeID, InstrumentID, Direction, Offset, Price, Volume, EntrustNo):
        ...
        req.OrderRef = str(self.OrderRef)  # 报单引用   本地会话全局唯一编号，必须保持递增；可由用户维护，也可由系统自动填写。一定为数字。
    ...
    def ExecOrder(self,order):
        ...
        offset = comm.OFFSET_DICT[order.openClose]
    ...
    def CloseAllOrder(self,symbol,pEntrustNo):
        # 必须按pEntrustNo 清仓,否则 母单找不到委托
        # 直接全平,不考虑订单映射
        rsp=[]
        positions_dict=self.getPosition(symbol)
        for longshort,vol in positions_dict.items():
            # InstrumentID, action, Offset, Volume, pEntrustNo
            if vol!=0:
                for n in range(vol):
                    rtnOrder=self.ExecOrder(symbol,longshort,comm.OFFSET_CLOSE,1,0)
                    rsp.append(rtnOrder)
        return rsp
    ...
    def OnRspQryOrder(self, pOrder: tdapi.CThostFtdcOrderField, pRspInfo: "CThostFtdcRspInfoField", nRequestID: "int",
        ...
        if pOrder is not None:
            ...
            if entrustNo in self.OrderDict:
                if self.OrderDict[entrustNo].status <= 3:
                    self.OrderDict[entrustNo].orderSysID = pOrder.OrderSysID.lstrip()
                    self.OrderDict[entrustNo].status = comm.STATUS_DICT[pOrder.OrderStatus]
    ...
    def OnRspQryTrade(self, pTrade: tdapi.CThostFtdcTradeField, pRspInfo: "CThostFtdcRspInfoField", nRequestID: "int",
        ...
        if pTrade is not None:
            ...
            entrustNo = int(pTrade.OrderRef)
            if entrustNo in self.OrderDict:
                if self.OrderDict[entrustNo].status <= 3:
                    self.OrderDict[entrustNo].bidPrice = pTrade.Price
                    self.OrderDict[entrustNo].bidVol = pTrade.Volume
    ...
    # 从持仓来比对数据可能由于策略修改而无法准确核算, 唯有从委托来核算
    def getFinishedOrderFromPid(self, pid):
        orders = []
        for order in self.OrderDict.values():
            if order.pid == pid and order.status == 4:
                orders.append(order)
        return orders
