...
class Mt5Api:
    ...
    def ExecOrder(self,order):
        ...
        if order.openClose == comm.TRADE_TYPE_CLOSE:
            ...
            result = self.sendOrder(comm.ACTION, order.symbol, order.askQty, comm.getSide(order.longShort, comm.TRADE_TYPE_CLOSE),
                               order.pEntrustNo,
        ...
        log.info(
            "mt5 order entrustNo:{} ,magic:{} ,symbol:{} ,vol:{} ,price:{} ,status:{} ,msg:{},".format(
                order.entrustNo, order.pEntrustNo, order.symbol, order.bidVol, order.bidPrice,order.status,order.statusMsg))
